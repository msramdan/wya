<div class="row mb-2">
    @if (!Auth::user()->roles->first()->hospital_id)
        <div class="col-md-6 mb-2">
            <label for="hospital_id">{{ trans('category-vendor/form.hospital') }}</label>
            <select class="form-control js-example-basic-multiple @error('hospital_id') is-invalid @enderror"
                ...
                <option value="" selected disabled>-- {{ trans('category-vendor/form.filter_hospital') }} --
                </option>
                @if (isset($categoryVendor))
                    @foreach ($hispotals as $hispotal)
                        <option value="{{ $hispotal->id }}"
                            {{ $categoryVendor->hospital_id == $hispotal->id ? 'selected' : '' }}
                            {{ $categoryVendor->hospital_id != $hispotal->id ? 'disabled' : '' }}>
                            {{ $hispotal->name }}
                        </option>
                    @endforeach
                @else
                    @foreach ($hispotals as $hispotal)
                        <option value="{{ $hispotal->id }}"
                            {{ isset($categoryVendor) && $categoryVendor->hospital_id == $hispotal->id ? 'selected' : (old('hospital_id') == $hispotal->id ? 'selected' : '') }}>
                            {{ $hispotal->name }}
                        </option>
                    @endforeach
                @endif
